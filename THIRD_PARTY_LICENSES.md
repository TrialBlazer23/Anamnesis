...
| David Chamberlain, Iliad/Odyssey recitations (hypotactic.com) — downloadable audio packs built by `pipeline/build_audio_pack.py`; manifest at `pipeline/data/audio/` | **CC BY — confirmed from the primary source**: "Audio and text annotations licensed as CC-BY, © 2016, 2017 by David Chamberlain" (evidence: `docs/licensing-evidence/hypotactic-homer-audio-cc-by.png`) | Attribution embedded in every pack's `manifest.json`: "Recitations © 2016, 2017 David Chamberlain (hypotactic.com), CC BY. Packaged unmodified." — files are not re-encoded |
